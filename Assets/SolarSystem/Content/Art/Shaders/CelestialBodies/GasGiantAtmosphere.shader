Shader "SolarSystem/Celestial/Gas Giant Atmosphere"
{
    Properties
    {
        [MainColor] _AtmosphereColor("Atmosphere Color", Color) = (0.76, 0.58, 0.42, 1)
        _RimPower("Rim Power", Range(0.5, 12)) = 4.8
        _RimIntensity("Rim Intensity", Range(0, 1)) = 0.2
        _NightsideVisibility("Nightside Visibility", Range(0, 1)) = 0.04
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 AtmosphereFragment(Varyings input) : SV_Target
            {
                ...
                half rim = pow(
                    saturate(1.0h - dot(normalWS, viewDirection)),
                    _RimPower);
